...
const table = [
  ...
  { label: "Módulo frontal", field: "moduloFrontal" },
  { label: "Ângulo de pressão frontal", field: "anguloFrontal" },
...
const calcMethod = (gear, module, angle) => {
  ...
  const moduloFrontal = module / Math.cos((angle * Math.PI) / 180);
  const anguloFrontal =
    Math.tan(Math.PI / 9) / Math.cos((angle * Math.PI) / 180);
...
const distanceCalcMethod = (driver, driven) => {
  return ((driver.dentes + driven.dentes) / 2) * driver.moduloFrontal;
};
...
export default () => {
  ...
  const [angleErr, setAngleErr] = useState(false);
  ...
  const calculate = () => {
    ...
    setAngleErr(isNaN(angleVal) || angleVal < 0 || angleVal > 45);

    const driverData = calcMethod(driverVal, moduleVal, angleVal);
    const drivenData = calcMethod(drivenVal, moduleVal, angleVal);
      ...
      <View style={styles.inputArea}>
        ...
        <View style={styles.inputContainer}>
          <Input
            style={styles.input}
            label="Ângulo"
            onChangeText={handleAngleChange}
            errorMessage={angleErr ? errAngle : ""}
          />
        </View>
